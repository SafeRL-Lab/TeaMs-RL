...
# from .inference import Call_model #main_model
from .inference_wizardlm import Call_model
from .openai_api import gpt3_call
...
folder = os.getcwd()[:-6] + 'data/' + '2SR_policy-'+ 'Generate_prompt/' + str(
...
with open(os.getcwd()[:-6] + 'data/' + '2SR_policy-'+ 'Generate_prompt/' + str(
        start_run_date_and_time) + '/all_state.json', 'w') as f:
    f.writelines('------------------ start ------------------' + '\n')
with open(os.getcwd()[:-6] + 'data/' + '2SR_policy-'+ 'Generate_prompt/' + str(
        start_run_date_and_time) + '/prompt_state.json', 'w') as f:
    f.writelines('------------------ start ------------------' + '\n')
...
class Evol_gpt_policy():
    ...
    def model(self, evol_prompt):   
        ...
        state = model_evaluate.evaluate(evol_prompt)        
    ...
    def step(self, action, instruction):
        ...
        reward_1 = 0
        cost_1 = 0
        judge_prompt_results_1 = self.model(judge_prompt_1)
        # print("judge_prompt_results_1---------:", judge_prompt_results_1)
        # if "Equal" == judge_prompt_results_1:
        #     cost_1 = -1
        if "Not Equal" == judge_prompt_results_1:
            reward_1 = 1

        
        # judge_prompt_2 = """
        #         If #the state prompt# contains “sorry” and is relatively short in length (i.e., less than 80 words):
        #         #the state prompt#: """ + state + """.
        #         Your Judgement 'Equal', Otherwise is 'Not Equal' (Must Just answer: Equal or Not Equal. No need to explain the reason; ‘Equal’ and ‘Not Equal’ are not allowed to appear simultaneously.):  
        #                     """
        # reward_2 = 0
        # cost_2 = 0
        # judge_prompt_results_2 = self.model(judge_prompt_2)
        # if "Equal" == judge_prompt_results_2:
        #     cost_2 = -1
        # if "Not Equal" == judge_prompt_results_2:
        #     reward_2 = 1
            
        # judge_prompt_3 = """
        #                 If #the state prompt# only contains punctuation and stop words:
        #                 #the state prompt#: """ + state + """.
        #                 Your Judgement is 'Equal', Otherwise is 'Not Equal' (Must Just answer: Equal or Not Equal. No need to explain the reason; ‘Equal’ and ‘Not Equal’ are not allowed to appear simultaneously.):  
        #                             """
        # reward_3 = 0
        # cost_3 = 0
        # judge_prompt_results_3 = self.model(judge_prompt_3)
        # if "Equal" == judge_prompt_results_3:
        #     cost_3 = -1
        # if "Not Equal" == judge_prompt_results_3:
        #     reward_3 = 1

        # judge_prompt_4 = """If #the state prompt# obviously copies some words from #the instruction prompt#, such as “given prompt”, “rewritten prompt”, “#Rewritten Prompt#”, etc.:
        #                         #the state prompt#: """ + state + """.
        #                         #the instruction prompt#: """ + instruction + """.
        #                         Your Judgement 'Equal', Otherwise is 'Not Equal' (Just answer: Equal or Not Equal. No need to explain the reason; ‘Equal’ and ‘Not Equal’ are not allowed to appear simultaneously.):  
        #                                     """
        # reward_4 = 0
        # cost_4 = 0
        # judge_prompt_results_4 = self.model(judge_prompt_4)
        # if "Equal" == judge_prompt_results_4:
        #     cost_4 = -1
        # if "Not Equal" == judge_prompt_results_4:
        #     reward_4 = 1
